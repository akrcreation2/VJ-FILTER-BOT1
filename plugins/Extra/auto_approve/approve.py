    ...
    if message.chat.id == AUTH_CHANNEL and join_db().isActive():
            ...
            for file in files:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton("🎀 𝙹𝙾𝙸𝙽 𝚁𝙴𝚀𝚄𝙴𝚂𝚃 𝙶𝚁𝙾𝚄𝙿 🎀", url="http://t.me/Knmlpro2_Group")
                    ],[
        ...
        if not files_:
            ...
            try:
                ...
                title = '@Knmlpro2  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
        ...
        files = files_[0]
        title = '@Knmlpro2  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                ...
                f_caption=f_caption
        if f_caption is None:
            f_caption = f"@Knmlpro2  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
        if not await db.has_premium_access(message.from_user.id):
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
